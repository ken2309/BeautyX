                    ...
                    <div className="booking-cnt__right">
                        <div className="booking-cnt__right-org">
                            <img
                                ...
                                alt="" className="org-avt" />
                            <div className="book-org-detail">
                                <p className="org-name">{org?.name}</p>
                                <p className="org-address">{org?.full_address}</p>
                            </div>
                        </div>
                            ...
                            <div className="branch-drop">
                                ...
                                >
                                    <span className='flex-row'>
                                        <img style={{ marginRight: "4px" }} src={icon.mapPinRed} alt="" />
                                        {bookTime.branch_id ?
                                            org?.branches?.find((i: any) => i.id === bookTime.branch_id)?.full_address
                                            :
                                            org?.full_address}
                                    </span>
                                    {
                                        org?.branches?.length > 0 && <img src={icon.arrowDownPurple} alt="" />
                                    }
                                </div>
                                {
                                    ...
                                    <ul ref={branchRef} className="branch-list">
                                        {
                                            branches?.map((item: any, index: number) => (
                                                <li
                                                    ...
                                                    style={bookTime.branch_id === item.id ? { color: "var(--text-black)" } : {}}
                                                    key={index}
                                                >
                                                    {item?.full_address}
                                                </li>
                        ...
                        <div className="flex-row-sp booking-cnt__right-time">
                            <div className="book-time">
                                <span className="book-section-title">Thời gian</span>
                                {
                                    (bookTime.time && bookTime.time) ?
                                        <span>{bookTime.date} {bookTime.time}</span>
                                        :
                        ...
                        <div className="booking-cnt__right-time">
                            <span className="book-section-title">Ghi chú</span><br />
                        ...
                        <div className="booking-cnt__bot">
                            <ButtonLoading
                                title={location.state?.TYPE === "BOOK_NOW" ? "Thanh toán và đặt hẹn ngay" : "Đặt hẹn ngay"}
                                loading={false}
