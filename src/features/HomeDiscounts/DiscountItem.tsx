    ...
    return (
        <div onClick={onDetail} className='home-discount-item__cnt'>
            ...
            <div className="home-discount-item__detail">
                <span className="name">{discountItem.productable.service_name}</span>
                <div className="flex-row price">
                    <span className="sale-price">{formatPrice(discountItem.view_price)}đ</span>
                    <span className="old-price">{formatPrice(discountItem.productable.price)}đ</span>
                </div>
                ...
                <div className="limit-bar">
                    <div
                        style={
                            !discountPar.total || discountPar.total === discountPar.used ?
                                { width: "100%" }
                                :
                                { width: `${discountPar.used / discountPar.total * 100}%` }
                        }
                        className="limit-bar__used"
                    >
                    </div>
                    <span className="limit-bar__num">
                        {discountPar?.total ? `Đã bán ${discountPar.used}/${discountPar.total}` : 'Đang mở bán'}
                    </span>
                </div>
            </div>
        </div>
