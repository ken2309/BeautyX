...
const paymentsSlice = createSlice({
    ...
    extraReducers: {
        [fetAsyncPaymentMethod.pending]: (state) => {
            return {
                ...state,
                PAYMENT: { ...state.PAYMENT, status: STATUS.LOADING },
            };
        },
        [fetAsyncPaymentMethod.fulfilled]: (state, { payload }) => {
            const CARD_ONLINE = state.PAYMENT_METHOD.find(
                (i: any) => i.id === 3
            );
            ...
            state.PAYMENT_METHOD = state.PAYMENT_METHOD.fill(
                CARD_ONLINE_NEW,
                0,
                1
            );
        ...
        [fetAsyncPaymentMethod.rejected]: (state) => {
            return {
                ...state,
                PAYMENT: { ...state.PAYMENT, status: STATUS.FAIL },
            };
        },
    },
});
